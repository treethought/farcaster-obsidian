...
export class Client {
	...
	async getChannelFeed(ids: string[]): Promise<CastsResponse> {
		let path = "/feed/channels" +
			`?with_recasts=true&channel_ids=${ids.join(",")
	...
	async react(hash: string, recast?: true): Promise<Response> {
		if (!this.settings.signerUUID) {
			throw new Error("No signer UUID set");
		}
		console.log("reacting cast: ");
		let path = `/reaction`;
		let url = this.settings.apiBaseUrl + path;

		let resp = await this.doRequest("POST", url, {
			signer_uuid: this.settings.signerUUID,
			reaction_type: recast ? "recast" : "like",
			target: hash,
		});
		...
		if (resp.status === 200) {
			...
			throw new Error(
				`Failed to publish reaction: ${resp.status} ${resp.statusText}`,
			);
		}
	}
}
